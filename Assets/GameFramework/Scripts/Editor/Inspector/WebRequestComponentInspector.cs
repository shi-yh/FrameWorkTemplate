...
namespace UnityGameFramework.Editor
{
    ...
    internal sealed class WebRequestComponentInspector : GameFrameworkInspector
    {
                        ...
                        if (GUILayout.Button("Export CSV Data"))
                        {
                            ...
                            if (!string.IsNullOrEmpty(exportFileName))
                            {
                                try
                                {
                                    ...
                                    foreach (TaskInfo webRequestInfo in webRequestInfos)
                                    {
                                        data[index++] = Utility.TextUtility.Format("{0},{1},{2},{3},{4}", webRequestInfo.Description, webRequestInfo.SerialId, webRequestInfo.Tag ?? string.Empty, webRequestInfo.Priority, webRequestInfo.Status);
                                    }

                                    File.WriteAllLines(exportFileName, data, Encoding.UTF8);
                                    Debug.Log(Utility.TextUtility.Format("Export web request task CSV data to '{0}' success.", exportFileName));
                                }
                                catch (Exception exception)
                                {
                                    Debug.LogError(Utility.TextUtility.Format("Export web request task CSV data to '{0}' failure, exception is '{1}'.", exportFileName, exception));
                                }
                            }
                        }
                    }
        ...
        {
            EditorGUILayout.LabelField(webRequestInfo.Description, Utility.TextUtility.Format("[SerialId]{0} [Tag]{1} [Priority]{2} [Status]{3}", webRequestInfo.SerialId, webRequestInfo.Tag ?? "<None>", webRequestInfo.Priority, webRequestInfo.Status));
        }
