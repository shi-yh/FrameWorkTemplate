...
namespace UnityGameFramework.Editor.ResourceTools
{
    ...
    internal sealed class ResourcePackBuilder : EditorWindow
    {
        ...
        private string GetVersionNameForDisplay(string versionName)
        {
            ...
            return Utility.TextUtility.Format("{0} ({1})", text, splitedVersionNames[splitedVersionNames.Length - 1]);
        }

        private void OnBuildResourcePacksStarted(int count)
        {
            Debug.Log(Utility.TextUtility.Format("Build resource packs started, '{0}' items to be built.", count));
            EditorUtility.DisplayProgressBar("Build Resource Packs", Utility.TextUtility.Format("Build resource packs, {0} items to be built.", count), 0f);
        }

        private void OnBuildResourcePacksCompleted(int successCount, int count)
        {
            int failureCount = count - successCount;
            string str = Utility.TextUtility.Format("Build resource packs completed, '{0}' items, '{1}' success, '{2}' failure.", count, successCount, failureCount);
            if (failureCount > 0)
            {
        ...
        {
            Debug.Log(Utility.TextUtility.Format("Build resource packs success, source version '{0}', target version '{1}'.", GetVersionNameForDisplay(sourceVersion), GetVersionNameForDisplay(targetVersion)));
            EditorUtility.DisplayProgressBar("Build Resource Packs", Utility.TextUtility.Format("Build resource packs, {0}/{1} completed.", index + 1, count), (float)index / count);
        }

        private void OnBuildResourcePackFailure(int index, int count, string sourceVersion, string targetVersion)
        {
            Debug.LogWarning(Utility.TextUtility.Format("Build resource packs failure, source version '{0}', target version '{1}'.", GetVersionNameForDisplay(sourceVersion), GetVersionNameForDisplay(targetVersion)));
            EditorUtility.DisplayProgressBar("Build Resource Packs", Utility.TextUtility.Format("Build resource packs, {0}/{1} completed.", index + 1, count), (float)index / count);
        }
    }
}
