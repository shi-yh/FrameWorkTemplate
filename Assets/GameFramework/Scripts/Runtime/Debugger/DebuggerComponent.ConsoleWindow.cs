        ...
        private sealed class ConsoleWindow : IDebuggerWindow
        {
            ...
            public void OnDraw()
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    m_InfoFilter = GUILayout.Toggle(m_InfoFilter, Utility.TextUtility.Format("Info ({0})", m_InfoCount), GUILayout.Width(90f));
                    m_WarningFilter = GUILayout.Toggle(m_WarningFilter, Utility.TextUtility.Format("Warning ({0})", m_WarningCount), GUILayout.Width(90f));
                    m_ErrorFilter = GUILayout.Toggle(m_ErrorFilter, Utility.TextUtility.Format("Error ({0})", m_ErrorCount), GUILayout.Width(90f));
                    m_FatalFilter = GUILayout.Toggle(m_FatalFilter, Utility.TextUtility.Format("Fatal ({0})", m_FatalCount), GUILayout.Width(90f));
                }
                ...
                GUILayout.BeginVertical("box");
                {
                    m_StackScrollPosition = GUILayout.BeginScrollView(m_StackScrollPosition, GUILayout.Height(100f));
                    {
                        if (m_SelectedNode != null)
                        {
                            ...
                            if (GUILayout.Button(Utility.TextUtility.Format("<color=#{0:x2}{1:x2}{2:x2}{3:x2}><b>{4}</b></color>{6}{6}{5}", color.r, color.g, color.b, color.a, m_SelectedNode.LogMessage, m_SelectedNode.StackTrack, Environment.NewLine), "label"))
                            {
                                CopyToClipboard(Utility.TextUtility.Format("{0}{2}{2}{1}", m_SelectedNode.LogMessage, m_SelectedNode.StackTrack, Environment.NewLine));
                            }
                        }
                    }
                    GUILayout.EndScrollView();
            ...
            private string GetLogString(LogNode logNode)
            {
                ...
                return Utility.TextUtility.Format("<color=#{0:x2}{1:x2}{2:x2}{3:x2}>[{4:HH:mm:ss.fff}][{5}] {6}</color>", color.r, color.g, color.b, color.a, logNode.LogTime.ToLocalTime(), logNode.LogFrameCount, logNode.LogMessage);
            }
