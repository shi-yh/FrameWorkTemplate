        ...
        private sealed class DataTable<T> : DataTableBase, IDataTable<T> where T : class, IDataRow, new()
        {
            ...
            public override bool AddDataRow(string dataRowString, object userData)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new GameFrameworkException(Utility.TextUtility.Format("Can not parse data row string for data table '{0}' with exception '{1}'.", new TypeNamePair(typeof(T), Name), exception), exception);
                }
            }
            ...
            public override bool AddDataRow(byte[] dataRowBytes, int startIndex, int length, object userData)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new GameFrameworkException(Utility.TextUtility.Format("Can not parse data row bytes for data table '{0}' with exception '{1}'.", new TypeNamePair(typeof(T), Name), exception), exception);
                }
            }
            ...
            private void InternalAddDataRow(T dataRow)
            {
                ...
                {
                    throw new GameFrameworkException(Utility.TextUtility.Format("Already exist '{0}' in data table '{1}'.", dataRow.Id, new TypeNamePair(typeof(T), Name)));
                }
