...
namespace GameFramework
{
    ...
    internal sealed partial class EventPool<T> where T : BaseEventArgs
    {
        ...
        public void Subscribe(int id, EventHandler<T> handler)
        {
            ...
            {
                throw new GameFrameworkException(Utility.TextUtility.Format("Event '{0}' not allow multi handler.", id));
            }
            else if ((m_EventPoolMode & EventPoolMode.AllowDuplicateHandler) != EventPoolMode.AllowDuplicateHandler && Check(id, handler))
            {
                throw new GameFrameworkException(Utility.TextUtility.Format("Event '{0}' not allow duplicate handler.", id));
            }
            else
            {
                m_EventHandlers.Add(id, handler);
        ...
        public void Unsubscribe(int id, EventHandler<T> handler)
        {
            ...
            {
                throw new GameFrameworkException(Utility.TextUtility.Format("Event '{0}' not exists specified handler.", id));
            }
        }
        ...
        private void HandleEvent(object sender, T e)
        {
            ...
            {
                throw new GameFrameworkException(Utility.TextUtility.Format("Event '{0}' not allow no handler.", e.Id));
            }
        }
    }
}
