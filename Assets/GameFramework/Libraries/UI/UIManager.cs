...
namespace GameFramework.UI
{
    ...
    internal sealed partial class UIManager : GameFrameworkModule, IUIManager
    {
        ...
        public int OpenUIForm(string uiFormAssetName, string uiGroupName, int priority, bool pauseCoveredUIForm, object userData)
        {
            ...
            if (uiGroup == null)
            {
                throw new GameFrameworkException(Utility.TextUtility.Format("UI group '{0}' is not exist.", uiGroupName));
            }
        ...
        public void CloseUIForm(int serialId, object userData)
        {
            ...
            if (uiForm == null)
            {
                throw new GameFrameworkException(Utility.TextUtility.Format("Can not find UI form '{0}'.", serialId));
            }
        ...
        private void LoadAssetFailureCallback(string uiFormAssetName, LoadResourceStatus status, string errorMessage, object userData)
        {
            ...
            m_UIFormsBeingLoaded.Remove(openUIFormInfo.SerialId);
            string appendErrorMessage = Utility.TextUtility.Format("Load UI form failure, asset name '{0}', status '{1}', error message '{2}'.", uiFormAssetName, status, errorMessage);
            if (m_OpenUIFormFailureEventHandler != null)
            {
